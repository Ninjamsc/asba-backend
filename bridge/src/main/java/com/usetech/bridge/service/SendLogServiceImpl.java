...
public class SendLogServiceImpl implements SendLogService {
    ...
    private final static Logger log = LoggerFactory.getLogger(SendLogServiceImpl.class.getSimpleName());
    ...
    public boolean send(LogBean logBean) {
        log.info("sending '{}' to '{}'", (Object) this.jmsTemplate.getDefaultDestinationName(),
                (Object) logBean);
        try {
            logBean.setFileContent(handleLog(logBean.getFileContent()));
            this.jmsTemplate.convertAndSend((Object) logBean);
        } catch (JmsException e) {
            log.error("Failed to send message using jmsTemplate: {}, {}", (Object) this.jmsTemplate, (Object) e);
            return false;
        }
        return true;
    }
