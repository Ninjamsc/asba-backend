...
public class TemplateBuilderServiceRestClient {
    ...
    private static final Logger log = LoggerFactory.getLogger(TemplateBuilderServiceRestClient.class);
    ...
    public PhotoTemplate getPhotoTemplateFromTevianCore(byte[] request, boolean isFirstAttempt) {
        ...
        try {
            ...
            log.debug("photoTemplate: {}", photoTemplate);

        } catch (ResourceAccessException e) {
            if (isFirstAttempt) {
                log.warn("Can't get photo template from Tevian core for the first time. Retrying...");
                photoTemplate = getPhotoTemplateFromTevianCore(request, false);
            } else {
                log.error("Can't get photo template from Tevian core for the N times.", e);
            }
        } catch (RestClientResponseException e) {
            log.error("BUILDER BIO TEMPLATE ERROR Code: {}", e.getRawStatusCode(), e);
            throw new TemplateBuilderServiceException(e.getResponseBodyAsString());

        } catch (Exception e) {
            log.error("Can't get photo template from Tevian core.", e);
        }
        return photoTemplate;
    }

    private PhotoTemplate getPhotoTemplateFromTechCore(byte[] request) {
        String url = getUrl();
        if (request == null) {
            return null;
        }
        log.debug("BUILDER BIO TEMPLATE request: {} URL: {}", request, url);
...
//                case 512:log.error("512 outOfMemory на GPU");
    ...
    private double[] getVectorFromByteArray(byte[] bytes) {
